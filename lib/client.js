...
export function addTag(repoDir, config) {
  if (!config) {
    return;
  }
  ...
  logger.info(`I'm going to create a new tag on the branch '${branchName}'.`);
  Repository.open(repoDir).then(repo => {
    repo.getBranchCommit(branchName).then(commit => {
      ...
      repo.createLightweightTag(commit.id(), tagName).then(ref => {
        logger.info(`The tag '${tagName}' has been created! Now you can do 'git push --tags' and 'bulldozer deploy'.`);
      });
    });
  }).catch(error => {
    if (error) {
